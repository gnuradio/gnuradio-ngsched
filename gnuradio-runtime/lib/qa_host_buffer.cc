...
BOOST_AUTO_TEST_CASE(t0)
{
    ...
    gr::buffer_sptr buf(
        gr::host_buffer::make_host_buffer(nitems, sizeof(int), nitems, nop, nop));
...
BOOST_AUTO_TEST_CASE(t1)
{
    ...
    for (int idx = 1; idx <= 16; ++idx)
    {
        ...
        // NOTE: This looks weird but is working as intended, space_available()
        // tries to reserve up to (nitems / 2) in case the data at the end of 
        // the buffer needs to be realigned.
        int expected = 0;        
        if (idx > 8)
            expected = nitems / 2;
        BOOST_CHECK(space == expected);
    }
...
BOOST_AUTO_TEST_CASE(t2)
{
    ...
    for (int idx = 1; idx <= 16; ++idx)
    {
        ...
        if (idx == 9)
        {
            ...
            // After filling the (nitems / 2) space buffer_single_mapped will
            // again report no space available
            space = buf->space_available();
            BOOST_CHECK(space == 0);
        }
    }
...
BOOST_AUTO_TEST_CASE(t3)
{
    ...
    for (int idx = 1; idx <= 16; ++idx)
    {
        ...
        // Reader 2 hasn't read anything so space avilable should remain 0
        BOOST_CHECK(buf->space_available() == 0);
    }        
    ...
    for (int idx = 1; idx <= 16; ++idx)
    {
        ...
        BOOST_CHECK(buf->space_available() == expected);
    }
}
...
BOOST_AUTO_TEST_CASE(t5)
{
    ...
    BOOST_CHECK(buf->space_available() == 8192);
    BOOST_CHECK(rdr1->items_available() == 384);
    
    buf->update_write_pointer(116);
    BOOST_CHECK(buf->space_available() == 8076);
    BOOST_CHECK(rdr1->items_available() == 384);
